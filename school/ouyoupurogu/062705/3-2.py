...
def ex06(pref, odays, fac,  epoch0, ndays, xlim, xticks, ylim, yticks, pngfile, log=False, rotation=28, pad=-2):
    ...
    covid.drawFrame(pref + "定点あたりの報告数の日別陽性者数への換算", optitle, ax, xlim, xticks, ylim, yticks, rotation=rotation, pad=pad)
    ax.bar([d[0] for d in weekly2["Week"]], weekly2[pref], label=f"定点あたりの報告数" r"$\time$"f"{fac}", **{**opbar, "width":7.0})
    ax.bar(fpmi["Week"], fac*np.array(fpmi[pref]), **{**opbar, "width":7.0, "color":"orange"})
    daily[p + "7days"] = daily[p].rolling(window=7).mean()

    ax.plot(daily["Date"]-np.timedelta64(odays, "D"), daily[p+'7days'], label="陽性者数7日間移動平均("r"$-$"f"{odays}日)", **opplot)
...
if __name__ == "__main__":
    ...
    xlim, xticks = [epoch0, epoch0+dt.timedelta(days=ndays)], [epoch0+dt.timedelta(days=i) for i in range(7, ndays, 4*7)]
    pref, odays, fac, ylim, yticks = "滋賀県", 3, 70, [8, 8000], [1, 10, 100]
