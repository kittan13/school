...
if __name__ == "__main__":
    # epoch0をnumpy.datetime64型に変換
    epoch0 = np.datetime64(dt.datetime(2022, 1, 10))
    ndays = 82 * 7
    xlim = [epoch0, epoch0 + np.timedelta64(ndays, "D")]
    xticks = [epoch0 + np.timedelta64(i, "D") for i in range(7, ndays, 4 * 7)]

    # 各都道府県に対する'fac'の値を設定
    pref_list = ["大阪府", "京都府", "東京都"]
    fac_values = [350, 500, 800]  # 好みに応じてこれらの値を変更
