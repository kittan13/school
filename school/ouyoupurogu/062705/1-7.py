...
def ex05(pref, epoch0, ndays, xlim, xticks, ylim, yticks, pngfile, log=False, rotation=28, pad=-2):
    ...
    ax = plt.axes([0.1, 0.12, 0.88, 0.75])
    ...
    ax.bar([d[0] for d in weekly2["Week"]], weekly2[pref], label=f"定点あたりの報告数", **{**opbar, "width": 7.0})
    ax.bar(fpmi["Week"], np.array(fpmi[pref]), **{**opbar, "width": 7.0, "color": "orange"})
    ax.legend(prop=goth, loc="upper right", bbox_to_anchor=(1.0, 1.0))

    # 全数把握の日別陽性者数の線グラフをプロット
    daily_data = covid.readCSV('\\\\wsl$\\Ubuntu-20.04\\home\\kittan\\ouyoupurogu\\062705\\newly_confirmed_cases_daily.csv', parse_dates=['Date'])
    dates = daily_data['Date']
    positive_cases = daily_data['ALL']
    ax.plot(dates, positive_cases, linewidth=1.5, color='red', label='全数把握の日別陽性者数')
    ax.legend(prop=goth, loc='upper left')
...
if __name__ == "__main__":
    ...
    xlim, xticks = [epoch0, epoch0 + dt.timedelta(days=ndays)], [epoch0 + dt.timedelta(days=i) for i in
                                                                  range(7, ndays, 4 * 7)]
