...
def ex06(pref, odays, fac, epoch0, ndays, xlim, xticks, ylim, yticks, pngfile, log=False, rotation=28, pad=-2):
    ...
    weekly = covid.readXLSXfp()
    weekly1, weekly2 = weekly[0]['Week'][3:7], {k: v[3:7] for k, v in weekly[0].items() if k != 'Week'}
    ...
    ax.bar([d.strftime("%Y-%m-%d") for d in weekly1], np.ravel(list(weekly2[pref])) * fac, label=f"定点あたりの報告数 × {fac}", **{**opbar, "width": 7.0})
