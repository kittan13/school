...
def ex06(pref, odays, fac, epoch0, ndays, xlim, xticks, ylim, yticks, pngfile, log=False, rotation=28, pad=-2):
    ...
    ax.set_title(pref + " 定点あたりの報告数の日別陽性者数への換算", fontproperties=goth, size=11, pad=4)
    ax.text(0, 1.04, "Y210249 田中勇気", transform=ax.transAxes, fontproperties=goth, size=8)  
...
if __name__ == "__main__":
    ...
    xlim, xticks = [epoch0, epoch0 + dt.timedelta(days=ndays)], [epoch0 + dt.timedelta(days=i) for i in range(7, ndays, 4 * 7)]
    pref, odays, fac, ylim, yticks = "滋賀県", 3, 70, [0.2, 2000], [1, 10, 100, 1000, 10000]
