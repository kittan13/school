...
def ex06(pref, odays, fac, epoch0, ndays, xlim, xticks, ylim, yticks, pngfile, log=False, rotation=28, pad=-2):
    ...
    xlim, xticks = [epoch0 + dt.timedelta(days=odays), epoch0 + dt.timedelta(days=ndays)], [epoch0 + dt.timedelta(days=i) for i in range(odays + 7, ndays, 4 * 7)]
    if log:
        ax.set_yscale("log")
    ...
    # 推定新規陽性者数の線グラフを描画
    dates = daily["Date"] - np.timedelta64(odays, "D")
    est_new_cases = daily[p] 
    ax.plot(dates, est_new_cases, label="推定新規陽性者数", **opplot_2)
    
    daily[p + "7days"] = pd.Series(daily[p]).rolling(window=7, min_periods=1).mean()  # 7日間移動平均を計算
    ax.plot(daily["Date"] - np.timedelta64(odays, "D"), daily[p + "7days"], label=f"陽性者数7日間移動平均 (-{odays}日)", **opplot_1)
    ax.legend(prop=goth, loc="best", bbox_to_anchor=(0.12, 0.2))
    plt.show()
...
if __name__ == "__main__":
    ...
    pref, odays, fac, ylim, yticks = "大阪府", 3, 70, [0.2, 200], [1, 10, 100, 1000, 10000, 100000,1000000]
